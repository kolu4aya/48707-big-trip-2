...
function createTaskTemplate(task) {
  ...
  function offersToString(offers) {
    return offers
      .map((items) => {
        return `<li class="event__offer">
          <span class="event__offer-title">${items.title}</span>&plus;&euro;&nbsp;
          <span class="event__offer-price">${items.price}</span>
        </li>`;
      })
      .join("");
  }
  ...
  duration =
    d > 0 ? `${d}D ${h}H ${m}M` : h > 0 ? `${h}H  ${m}M` : m > 0 ? `${m}M` : 0;
