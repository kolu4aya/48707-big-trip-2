...
function createEditPointTemplate(data) {
  const {type, destination, offers, timeStart, timeEnd, favorite, cost} = data;

  function typesToString(items = TYPE_OF_ROUTE) {
  return items
    .map((item) => {
      let typeLow = item.toLowerCase();
      let checked = item === type ? ' checked' : '';
  ...
  return items
    .map((item) => {
      let itemAtr = item.title.split(item.title.split(' ').length - 1);
      let checked = offers.includes(item) ? ' checked' : '';
